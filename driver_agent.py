...
class DriverAgent(mesa.Agent):
    def __init__(self, unique_id, model, driverType, layerLevel = 1):
        ...
        probOfBadDriver = random.randint(1,10)

        if probOfBadDriver <= 5:
            self.AverageMexicoCityDriver()
    ...
    def AverageMexicoCityDriver(self):
        self.velocity = 1
        self.driverType = 5
        self.isPriority = True
        self.layerLevel = 5
        ...
        for agent in cellmates:
            ...
            for newAgent in newMoveMates:
                if(type(newAgent) == SmartTrafficLightAgent and newAgent.color == "red"):
                    ...
                    if(self.driverType == 5):
                        probabilityOfCrash = random.randint(1,10)
                        if probabilityOfCrash <= 7:
                            return self.getNextPosition(agent)
                        else:
                            return(self.pos)
                    return (self.pos)

                elif ((type(newAgent) == IntersectionTrafficLightsAgent) or type(newAgent) == DriverAgent) and (self.driverType == 5):  
                    probabilityOfCrash = random.randint(1,10)
                    if probabilityOfCrash <= 7:
                        return self.getNextPosition(agent)
                    else:
